...
const PROFILES = [
  {
    id: 'factory_24_7',
    title: 'Usine 24h/7',
    desc: 'Production continue, forte autoconsommation.',
    icon: (
  ...
  {
    id: 'office_8_18',
    title: 'Bureaux 8h-18h',
    desc: 'Charge diurne alignée sur le soleil.',
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6 text-[#4F7CAC]">
  ...
  {
    id: 'hotel_seasonal',
    title: 'Hôtel / Saisonnier',
    desc: 'Charge variable, pics estivaux.',
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6 text-[#4F7CAC]">
  ...
  {
    id: 'weekend_closed',
    title: 'Fermé le week-end',
    desc: 'Charge industrielle classique, surplus weekend.',
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6 text-[#4F7CAC]">
...
export default function OnboardingWizard() {
  ...
  const handleNext = () => {
    if (step === 1 && selectedCity) setStep(2);
    else if (step === 2 && selectedProfile) setStep(3);
    else if (step === 3 && isBillValid) {
      completeOnboarding({
        city: selectedCity,
      ...
      <div className="w-full max-w-[640px] bg-white/75 backdrop-blur-xl border border-[rgba(210,222,240,0.90)] shadow-[0_8px_32px_rgba(50,80,130,0.10)] rounded-2xl p-8 flex flex-col transition-all duration-300">
        ...
        {step === 1 && (
          <div className="flex flex-col animate-in fade-in slide-in-from-right-4 duration-300">
            <h2 className="text-xl font-bold text-text-heading mb-1">Sélectionnez votre hub industriel</h2>
            <p className="text-sm text-text-muted mb-6">Données météo NASA POWER adaptées à votre région.</p>
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              >
                <span>{selectedCity ? selectedCity.name : 'Choisir une ville...'}</span>
                ...
                <div className="absolute top-full left-0 right-0 mt-2 bg-white/90 backdrop-blur-xl border border-[rgba(210,222,240,0.90)] rounded-xl shadow-lg z-20 overflow-hidden flex flex-col max-h-60">
                  ...
                  <div className="overflow-y-auto custom-scrollbar">
                    ...
                    {filteredCities.length === 0 && (
                      <div className="px-4 py-3 text-sm text-text-muted text-center">Aucun résultat</div>
                    )}
                  </div>
                </div>
              )}
        ...
        {step === 2 && (
          <div className="flex flex-col animate-in fade-in slide-in-from-right-4 duration-300">
            <h2 className="text-xl font-bold text-text-heading mb-6">Quel est le rythme opérationnel ?</h2>
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-text-muted leading-relaxed">{profile.desc}</p>
                    </div>
                  </div>
                );
              })}
        ...
        {step === 3 && (
          <div className="flex flex-col animate-in fade-in slide-in-from-right-4 duration-300">
            <h2 className="text-xl font-bold text-text-heading mb-1">Dimensionnement par facture</h2>
            <p className="text-sm text-text-muted mb-6">Nous estimons la puissance cible à partir de votre consommation ONEE.</p>
            
            <div className="flex flex-col gap-6">
              <div>
                <label className="block text-xs font-semibold text-text-muted mb-2 uppercase tracking-wide">Facture mensuelle moyenne (MAD)</label>
                <input 
                  type="number"
                  min="0"
              ...
              <div className="bg-canvas-from/50 border border-[rgba(210,222,240,0.60)] rounded-xl p-5 flex flex-col gap-3">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-text-body font-medium">Consommation estimée :</span>
                  <span className="text-sm text-brand font-bold tabular-nums">
                    {isBillValid ? estimatedMonthlyKwh.toLocaleString('fr-MA', { maximumFractionDigits: 0 }) : '0'} <span className="text-xs text-text-muted font-medium">kWh/mois</span>
                  </span>
                </div>
                <div className="border-t border-[rgba(210,222,240,0.40)]"></div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-text-body font-medium">Puissance cible suggérée :</span>
        ...
        <div className="mt-8 pt-6 border-t border-[rgba(210,222,240,0.60)] flex justify-between items-center">
          ...
          >
            {step === 3 ? 'Générer l\'audit' : 'Continuer'}
          </button>
        </div>
